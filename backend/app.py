...
# Added specific Google error imports for robust handling
...
def analyze(
    ...
    try:
        ...
        # Catch 404 (Model not found) or 400 (Bad Request)
        print(f"Gemini Client Error: {e}")
        raise HTTPException(
            status_code=400, 
            detail=f"Gemini API Error: The model '{modelId}' was not found or is invalid. Please check your model settings."
        )
        ...
        raise HTTPException(
            status_code=503, 
            detail="Gemini AI is currently overloaded (503). Please wait a moment and try again."
        )
